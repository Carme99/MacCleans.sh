...
- **iOS backup permission error** - Fixed script exiting silently when `find` returns "Operation not permitted" on iOS backup directory by adding `|| true` to prevent `set -e` from triggering on expected permission errors
