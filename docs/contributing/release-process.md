...
3. **Run the release helper** from a `main` checkout (the script refuses to run on any other branch):
   ...
   - `--check` (added v5.7.0) runs the same state verification without making changes; used by the CI gate and for a "would the release prep do anything?" sanity check. Exits 0 on clean, 1 on drift.
